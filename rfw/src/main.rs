...
async fn run_firewall(opt: RunOpt) -> anyhow::Result<()> {
    ...
    // 附加 XDP 程序到网络接口
    // auto 模式：先尝试默认模式（Native），失败则自动回退到 SKB 兼容模式
    let actual_mode = if xdp_mode == "auto" {
        match program.attach(&iface, xdp_flags) {
            Ok(_) => "auto/native".to_string(),
            Err(e) => {
                warn!(
                    "默认 XDP 模式附加失败（{}），自动回退到 SKB 兼容模式...",
                    e
                );
                program.attach(&iface, XdpFlags::SKB_MODE).context(format!(
                    "SKB 兼容模式也无法附加到接口 {}，请检查内核版本（需要 5.15+）",
                    iface
                ))?;
                "auto/skb-fallback".to_string()
            }
        }
    } else {
        program.attach(&iface, xdp_flags).context(format!(
    ...
    info!("XDP 程序已成功附加到接口: {} (模式: {})", iface, actual_mode);
